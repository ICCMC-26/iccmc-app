...
const I18N={
  ar:{dir:'rtl', other:'English',
    ...
    pv_generated:'صدر بتاريخ', pv_conf:'سري — للاستخدام الداخلي', pv_page:'صفحة', pv_contents:'المحتويات',
  ...
  en:{dir:'ltr', other:'العربية',
    ...
    pv_generated:'Generated', pv_conf:'Confidential — internal use only', pv_page:'Page', pv_contents:'Contents',
...
let BOARD_TBL=false;
/* Latin digits in BOTH languages. Arabic-Indic numerals were the odd one out: every other
   figure in this app already renders Latin, and passport and visa numbers are Latin on the
   documents themselves. Two digit scripts on one screen makes figures harder to compare at
   a glance, which is this page's entire job. */
function bdN(n){ return Number(n||0).toLocaleString('en-US'); }
...
async function buildDossier(P,VS,LG,opts){
  ...
  const foot=n=>`<div class="pv-foot"><span class="pgn" dir="ltr">${n} / ${total}</span></div>`;
  ...
  const cover=`<div class="pg cover">
      ...
      <div class="cv-mid"><div class="cv-kicker">${t('pv_title')}</div>
        ...
        ${p.passport_no?`<div class="cv-id" dir="ltr">${esc(p.passport_no)}</div>`:''}
        <div class="cv-meta"><span class="cv-nat">${nat}</span></div></div>
      <div class="cv-foot"><div>${t('pv_generated')} ${today}</div><div class="cv-conf">${t('pv_conf')}</div><div class="pgn">${t('pv_page')} <span dir="ltr">1 / ${total}</span></div></div></div>`;
...
/* ═══ PHONE LAYER (v302) — see app.css. The header's secondary actions in a bottom sheet behind «⋯»;
   ...
   The A4 builder sheet keeps its true size on a phone and pans left/right (Ibrahim, v305). */
...
/* a table value never clips (v305): when the text is wider than its cell the type steps down (to 8pt at
   the least). The WhatsApp kiosk applies the same rule (fitCell), so both papers read alike. */
function istFitCell(inp){
  inp.style.fontSize='';
  let pt=11;
  while(inp.scrollWidth>inp.clientWidth+1 && pt>8){ pt-=0.5; inp.style.fontSize=pt+'pt'; }
}
function istFitCells(){ const tb=$('#ist-tbody'); if(tb) tb.querySelectorAll('.ist-hin,.ist-ein').forEach(istFitCell); }
function istRenderRows(){
  ...
  tb.querySelectorAll('.ist-hin').forEach(inp=>inp.oninput=()=>{ const r=_IST.rows[+inp.dataset.ri];
    ...
    inp.classList.remove('ist-pre'); istFitCell(inp); });
  tb.querySelectorAll('.ist-ein').forEach(inp=>inp.oninput=()=>{ const r=_IST.rows[+inp.dataset.ri];
    if(r){ r[inp.dataset.rk]=inp.value; _IST._dirty=true; } istFitCell(inp); });
  istFitCells();
...
function selOpen(){
  ...
  openPicker({host:document.body, title:t('sel_h'), isIn:()=>false, actions:[
    {label:n=>t('sel_dossier_n',n), tip:t('sel_dossier_tip'), cls:' quiet', run:async rows=>{ istPkClose(); await exportDossierZip(rows, document.body); return false; }},
    {label:n=>t('sel_bundle_n',n), tip:t('sel_bundle_tip'), cls:'', run:async rows=>{ istPkClose();
...
function openPicker(opts){
  ...
  m.innerHTML=`<div class="pk-box" role="dialog" aria-label="${esc(t('ist_pk_h'))}">
    ...
    <div class="pk-ft"><span class="pk-count" id="pk-count">${esc(t('ist_pk_sel',0))}</span><span class="spacer"></span>
      <button class="add quiet pk-cancel">${esc(t('ist_pk_cancel'))}</button>${(opts.actions||[{label:opts.addLabel,cls:''}]).map((a,i)=>`<button class="add pk-add${a.cls||''}" data-act="${i}" title="${esc(a.tip||'')}" disabled>${esc(a.label(0))}</button>`).join('')}</div></div>`;
